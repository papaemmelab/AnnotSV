...
proc checkPathogenicNRSVfile {} {
    ...
    #######################################################################
    regsub "Sources/?" $g_AnnotSV(sourcesDir) "Annotations/FtIncludedInSV/dbVar_pathogenic_NR_SV/$g_AnnotSV(genomeBuild)" NRSVdir
    ...
    ################################
    regsub "Sources/?" $g_AnnotSV(sourcesDir) "Annotations/FtIncludedInSV/dbVar_pathogenic_NR_SV/$g_AnnotSV(genomeBuild)" dbVarDir
    set dbVarFileDownloaded1 "$dbVarDir/$g_AnnotSV(genomeBuild).nr_deletions.tsv.gz"
    set dbVarFileDownloaded2 "$dbVarDir/$g_AnnotSV(genomeBuild).nr_duplications.tsv.gz"
	...
	set dbVarBedFile "$dbVarDir/[clock format [clock seconds] -format "%Y%m%d"]_dbVar_pathogenic_NR_SV.bed"
	set dbVarBedFileFormatted "$dbVarDir/[clock format [clock seconds] -format "%Y%m%d"]_dbVar_pathogenic_NR_SV.formatted.bed"
	set dbVarBedFileFormattedSorted "$dbVarDir/[clock format [clock seconds] -format "%Y%m%d"]_dbVar_pathogenic_NR_SV.formatted.sorted.bed"
	set dbVarHeaderFile "$dbVarDir/[clock format [clock seconds] -format "%Y%m%d"]_dbVar_pathogenic_NR_SV.header.tsv"
...
proc pathogenicNRSVannotation {SVchrom SVstart SVend} {
...
##	    
	...
	foreach SVtoAnn [array names L_event] {
	    ...
	    set pathogenicNRSVtext($SVtoAnn) "[join $L_event($SVtoAnn) ";"]\t[join $L_variant($SVtoAnn) ";"]\t[join $L_status($SVtoAnn) ";"]"
	}
