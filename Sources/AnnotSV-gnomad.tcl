...
proc gnomADannotation {SVchrom SVstart SVend SVtype} {
	...
	foreach svtype {"DUP" "DEL" "INV" "INS"} {
		...
		# - INSERTION (ALU, INS, LINE1, SVA) (length < 50bp)
		...
		if {$SVtoAnn_length<=1} {
		    ...
		    # we keep only those with > 70% (default) overlap with the SV to annotate
	...
	foreach svtype {"DUP" "DEL" "INV" "INS" "None"} {
		...
		foreach SVtoAnn [lsort -unique $L_allSVtoAnn] {
			...
			# headerOutput "GD_ID GD_AN GD_N_HET GD_N_HOMALT GD_AF GD_POPMAX_AF GD_ID_others"
			set gnomADtext($SVtoAnn,$svtype) "[join $L_GD_ID($SVtoAnn,$svtype) ";"]\t[join $L_GD_AN($SVtoAnn,$svtype) ";"]\t[join $L_GD_N_HET($SVtoAnn,$svtype) ";"]\t[join $L_GD_N_HOMALT($SVtoAnn,$svtype) ";"]\t$max_AF\t$max_POPMAX"
		    } else {
			set gnomADtext($SVtoAnn,$svtype) "\t\t\t\t-1\t-1"
		    }
		    ...
		    append gnomADtext($SVtoAnn,$svtype) "\t[join $L_GD_ID_other($SVtoAnn,$svtype) ";"]"
		}
	    }
	}
