...
proc ExternalAnnotations args {
    ...
    # All $ANNOTSV/Annotations/*/*.tsv and $ANNOTSV/Annotations/*/*.tsv.gz files are used (defined in AnnotSV-main.tcl)
    #
	...
	if {![info exists g_ExtAnnotation($File_Anno,Loaded)]} {
	    ...
	    puts "\t\t([llength [set g_ExtAnnotation($File_Anno,L_ID)]] gene identifiers and [expr {[llength [split [set g_ExtAnnotation($File_Anno,Header)] "\t"]]-1}] annotations columns: [join [lrange [split [set g_ExtAnnotation($File_Anno,Header)] "\t"] 1 end] ", "])"
