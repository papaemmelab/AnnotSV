...
proc DGVannotation {SVchrom SVstart SVend} {
	...
	foreach SVtoAnn [array names remember] {
	    # DGV_GAIN_IDs
	    set dgvText($SVtoAnn) [join $DGVgainID($SVtoAnn) ","]
	    ...
	    append dgvText($SVtoAnn) "\t$n_samples_with_SV"
		...
		append dgvText($SVtoAnn) "\t$n_samples_tested"

		# DGV_GAIN_Frequency
		set freq [expr {$n_samples_with_SV*1.0/$n_samples_tested}]
		set freq [format "%11.8f" $freq]
		if {[set g_AnnotSV(metrics)] eq "fr"} {regsub {\.} $freq "," freq}
		append dgvText($SVtoAnn) "\t$freq"
	    } else {
		append dgvText($SVtoAnn) "\t0\t-1"
	    }

	    # DGV_LOSS_IDs
	    append dgvText($SVtoAnn) "\t[join $DGVlossID($SVtoAnn) ","]"
