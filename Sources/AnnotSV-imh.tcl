...
proc IMHannotation {SVchrom SVstart SVend SVtype} {
	...
	foreach svtype {"DUP" "DEL" "INV" "INS" "None"} {
		...
		foreach SVtoAnn [lsort -unique $L_allSVtoAnn] {
			...
			# headerOutput "IMH_ID IMH_AF IMH_ID_others"
			set IMHtext($SVtoAnn,$svtype) "[join $L_IMH_ID($SVtoAnn,$svtype) ";"]\t$max_AF"
		    } else {
			set IMHtext($SVtoAnn,$svtype) "\t-1"
		    }
		    ...
		    append IMHtext($SVtoAnn,$svtype) "\t[join $L_IMH_ID_other($SVtoAnn,$svtype) ";"]"
		}
	    }
	}
	set IMHtext(DONE) 1	
