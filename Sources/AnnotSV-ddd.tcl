...
proc DDDfrequencyAnnotation {SVchrom SVstart SVend} {
	...
	while {![eof $f]} {
	    ...
	    set DDDSV_length [expr {$DDDSV_end-$DDDSV_start}]
		...
		if {$g_AnnotSV(reciprocal) eq "yes"} {			
		    if {[expr {$overlap_length*100.0/$DDDSV_length}] < $g_AnnotSV(overlap)} {continue}
		}
	    }
	...
	foreach SVtoAnn [array names L_SVddd] {
	    ...
	    set dddText($SVtoAnn) "[join $L_SVddd($SVtoAnn) ";"]"
	    set max -1
	    foreach a $L_DUPn($SVtoAnn) {if {$a > $max} {set max $a}}
	    append dddText($SVtoAnn) "\t$max"
	    set max -1
